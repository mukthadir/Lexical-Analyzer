...
int getToken()
{
	...
	switch (c)
	{
		case '.': ttype = DOT;	*token = c;		return ttype;
		case '+': ttype = PLUS;	*token = c;		return ttype;
		case '-': ttype = MINUS;*token = c;		return ttype;
		case '/': ttype = DIV;	*token = c;		return ttype;
		case '*': ttype = MULT;	*token = c;		return ttype;
		case '=': ttype = EQUAL;*token = c;		return ttype;
		case ':': ttype = COLON;*token = c;		return ttype;
		case ',': ttype = COMMA;*token = c;		return ttype;
		case ';': ttype = SEMICOLON;*token = c;		return ttype;
		case '[': ttype = LBRAC;*token = c;		return ttype;
		case ']': ttype = RBRAC;*token = c;		return ttype;
		case '(': ttype = LPAREN;*token = c;		return ttype;
		case ')': ttype = RPAREN;*token = c;		return ttype;
		case '<':
			c = getchar();
			if (c == '=')
				{ttype = LTEQ;*token = c;}
			else if (c == '>')
				{ttype = NOTEQUAL;*token = c;}
			else
			{
